...
data <- data %>%
  filter((year == 2015) & (status_alive == 1)) %>%
  # look at firms below 10m euro revenues and above 10000 euros
  filter(!(sales_mil > 10)) %>%
  filter(!(sales_mil < 0.01))

Hmisc::describe(data$fastgrowth)
write_csv(data,paste0(output,"bisnode_fastgrowth2015.csv"))

###########################################################
# Feature engineering
###########################################################

data <- read_csv(paste0(data_out,"bisnode_fastgrowth2015.csv"))
...
#Here from balancesheet we will not use variable for intang_assets because majority is zeros
pl_names <- c("extra_exp","extra_inc",  "extra_profit_loss", "inc_bef_tax" ,"inventories",
              "material_exp", "profit_loss_year", "personnel_exp")
bs_names <- c( "curr_liab", "fixed_assets", "liq_assets", "curr_assets",
...
data <- data %>%
  mutate(fastgrowth_f = factor(fastgrowth, levels = c(0,1)) %>%
           recode(., `0` = 'no_fast', `1` = "fast"))
...
ggplot(data = data, aes(x=sales_mil_log, y=as.numeric(fastgrowth))) +
  ...
  geom_smooth(method="loess", se=T, colour=color[5], size=1.5, span=0.9) +
  labs(x = "sales_mil_log",y = "fastgrowth") +
  theme_classic()


ols_s <- lm(fastgrowth~sales_mil_log+sales_mil_log_sq,
                data = data)
summary(ols_s)
...
#Looks awful that is why d1_sales_mil_log was modified
ggplot(data = data, aes(x=d1_sales_mil_log, y=as.numeric(fastgrowth))) +
...
data <- data %>%
  filter(!is.na(liq_assets_bs),!is.na(foreign), !is.na(ind), !is.na(age),!is.na(foreign), !is.na(material_exp_pl), !is.na(m_region_loc))
...
ggplot(data = data, aes(x=d1_sales_mil_log_mod, y=as.numeric(fastgrowth))) +
  geom_point(size=2,  shape=20, stroke=2, fill="blue", color="blue") +
  geom_smooth(method="loess", se=F, colour="black", size=1.5, span=0.9) +
  labs(x = "d1_sales_mil_log",y = "fastgrowth") +
  theme_classic() +
...
#filter additional missing values and firms that exited in 2015
data <- data %>%
