...
#location folders
data_in <- paste0(dir,"cases_studies_public/bisnode-firms/clean/")
data_out <- paste0(dir,"textbook_work/ch17/bisnode-firms/")
output <- paste0(dir,"textbook_work/ch17/bisnode-firms/output/")
...
data <- data %>%
  group_by(comp_id) %>%
  mutate(default = ((status_alive == 1) & (Lag(status_alive, -2) == 0)) %>%
           as.numeric(.)) %>%
  ungroup()
